...
class data_sampler_CFRL(object):
                ...
                for i, line in enumerate(f):
                    ...
                    if (len(items[0]) > 0):
                        ...
                        if items[1] != 'noNegativeAnswer':
                            ...
                            sample['oie'] = json.loads(items[9]) if len(items) >= 10 else None
    ...
    def _tokenize_hybridprompt(self, sample):
        ...
        if sample['oie']:
            ...
            if not trigger_token_indices:
                print(f"relation:{sample['relation']} -- oie:{sample['oie']}---trigger_token_indices: {trigger_token_indices}--text: {' '.join(prompt)}")
