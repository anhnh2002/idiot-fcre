...
class data_sampler_CFRL(object):
    def __init__(self, config=None, seed=None):
        ...
        self.na_id_2 = self.config.na_id_2

        tmp = self._read_na_data(self.config.training_data, self._temp_datapath('train'))
        self.training_na_data, self.training_na_data_2 = tmp["na"], tmp["na_2"]
        tmp = self._read_na_data(self.config.valid_data, self._temp_datapath('valid'))
        self.valid_na_data, self.valid_na_data_2 = tmp["na"], tmp["na_2"]
        tmp = self._read_na_data(self.config.test_data, self._temp_datapath('test'))
        self.test_na_data, self.test_na_data_2 = tmp["na"], tmp["na_2"]

        self.na_rel = self.id2rel[self.na_id]
        self.na_rel_2 = self.id2rel[self.na_id_2]
    ...
    def __next__(self):
        ...
        if self.na_rel_2 not in self.seen_relations:
            self.seen_relations.append(self.na_rel_2)
            self.history_test_data[self.na_rel_2] = cur_na_test_data_2
            self.seen_descriptions[self.na_rel_2] = self.id2des[self.na_id_2]
        else:
            self.history_test_data[self.na_rel_2] += cur_na_test_data_2
        ...
        current_relations.append(self.na_rel_2)
        cur_training_data[self.na_rel_2] = cur_na_training_data_2
        cur_valid_data[self.na_rel_2] = cur_na_valid_data_2
        cur_test_data[self.na_rel_2] = cur_na_test_data_2
    ...
    def _read_na_data(self, file: str, save_data_path):
        ...
        if os.path.isfile(save_data_path):
            ...
            processed_na_data = {
                "na": [[] for i in range(self.config.num_of_relation)],
                "na_2": [[] for i in range(self.config.num_of_relation)]
            }
                ...
                for line in na_data[key]:
                    ...
                    if (len(items[0]) > 0):
                        ...
                        if items[1] != 'noNegativeAnswer':
                            ...
                            if sample['relation'] == self.na_id:
                                sample['relation_definition'] = items[10] if "train" in file else None
                                processed_na_data["na"][int(key)].append(self.tokenize(sample))
                            elif sample['relation'] == self.na_id_2:
                                sample['relation_definition'] = self.id2des[self.na_id_2] if "train" in file else None
                                processed_na_data["na_2"][int(key)].append(self.tokenize(sample))
                            else:
                                raise ValueError("NA relation not found")
